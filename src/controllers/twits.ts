...
import {
  InsertSnapshare,
  SelectSnapshare,
} from "../db/schemas/snapshareSchema";
import {
  editTwitSnapSchema,
  feedSchema,
  hashtagSchema,
...
router.get("/search", async (req, res, next) => {
  try {
    ...
    const similarTwitSnaps = await twitSnapsService.getTwitSnapsBySimilarity(
      result.q
    );
...
router.post("/feed", async (req, res, next) => {
  try {
    ...
    const feed = await twitSnapsService.getFeed(
      date,
      result.limit,
      result.followeds
    );
...
router.post("/:id/reply", async (req, res, next) => {
  try {
    ...
    const newTwitSnapReply: InsertTwitsnap = {
      ...result,
      parentId: twitsnapId,
    };
    const newTwitSnap: SelectTwitsnap | null =
      await twitSnapsService.createTwitSnap(newTwitSnapReply);
    res.status(201).json(newTwitSnap);
  } catch (err: unknown) {
    next(err);
  }
...
router.get("/:id/replies", async (req, res, next) => {
  try {
    const twitsnapId = req.params.id;
    const twitSnapReplies =
      await twitSnapsService.getTwitSnapReplies(twitsnapId);
...
router.post("/:id/share", async (req, res, next) => {
  try {
    ...
    const newSnapShare: SelectSnapshare | null =
      await twitSnapsService.createSnapshare(snapShare);
...
router.post("/:id/mention", async (req, res, next) => {
  try {
    ...
    const twitsnapId = req.params.id;
    const body = await twitSnapsService.mentionUser(
      twitsnapId,
      result.mentionedUser
    );
...
router.delete("/:id/mention", async (req, res, next) => {
  try {
    ...
    const twitsnapId = req.params.id;
    await twitSnapsService.deleteTwitSnapMention(
      twitsnapId,
      result.mentionedUser
    );
    res.status(204).send();
...
router.post("/:id/favourite", async (req, res, next) => {
  try {
    ...
    const twitsnapId = req.params.id;
    const output = await twitSnapsService.postFavourite(
      twitsnapId,
      result.userId
    );
